                ...
                const mailOptions = {
                    ...
                    html: `
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                            ...
                            <p>A login attempt was made to your account. Please use the following verification code to complete your login for:</p>
...
// ** Get User Info from Reset Token **
app.get('/api/auth/reset-token-info/:token', async (req, res) => {
    try {
...
app.put('/api/classrooms/:classId/seating', authMiddleware, async (req, res) => {
    const { classId } = req.params;
    const { seatingPositions, assignedUsers, studentScores, chairGroups } = req.body; // ✨ Added chairGroups
    try {
        console.log('Seating update request:', { seatingPositions, assignedUsers, studentScores, chairGroups });
        ...
        if (chairGroups) updateObj.chairGroups = chairGroups; // ✨ Save chairGroups
        ...
        res.json({
            ...
            chairGroups: classroom.chairGroups // ✨ Return chairGroups
        });
    } catch (err) {
